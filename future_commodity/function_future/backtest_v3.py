...
def analyze_pos_distribution(merged_data):
    ...
    max_dd = calc_dd(merged_data.groupby('date').date_cum_ret.last().cumsum() + 1).min()
    calmar = merged_data.groupby('date').date_cum_ret.last().mean() * 252 / abs(max_dd / 100)
    ...
    stats = {
        ...
        "日均交易次数（开平算一次）": f"{trade_count / date_count:.2f}",
...
def is_in_exclude_period(dt, trading_hours):
    """判断是否在要排除的时间段内"""
    ...
    else: print('period incorrect')

# def plot_group_bar(merged_data, period, trading_hours, g=21):
#     merged_data['rtn'] = np.log1p(merged_data['tick6t60avg'].pct_change(period).shift(-(period+1)))
#     merged_data['factor_decile'] = pd.qcut(merged_data['factor'], q=g, labels=False, duplicates='drop') - g // 2
#     merged_data['exclude'] = merged_data['datetime'].apply(is_in_exclude_period, args=(trading_hours,))
#     filtered_data = merged_data[~merged_data['exclude']].drop(columns=['exclude'])

#     return filtered_data.groupby('factor_decile')['rtn'].sum().plot(kind='bar')


def plot_group_bar(merged_data, periods, trading_hours, g=21, figsize=(8, 4)):
    """
    绘制多个周期的因子分组收益柱状图
    ...
    # 因子分组
    df['factor_decile'] = pd.qcut(df['factor'], q=g, labels=False, duplicates='drop') - g // 2
    
    # 排除交易时间
    if trading_hours:
        df['exclude'] = df['datetime'].apply(is_in_exclude_period, args=(trading_hours,))
        df = df[~df['exclude']].drop(columns=['exclude'])
    
    # 计算各周期的收益率
    rtn_cols = []
    for period in periods:
        rtn_col = f'rtn_{period}'
        df[rtn_col] = np.log1p(df.groupby('date')['tick6t60avg'].pct_change(period).shift(-(period+1)))
        rtn_cols.append(rtn_col)
    
    # 按因子分组计算各周期的收益率和
    group_results = {}
    for rtn_col, period in zip(rtn_cols, periods):
        group_sum = df.groupby('factor_decile')[rtn_col].sum()
        group_results[f'period{period}'] = group_sum
    ...
    # 绘制分组柱状图
    bar_width = 0.8 / len(periods)  # 根据周期数量调整柱宽
    indices = np.arange(len(result_df))
    
    for i, (period, col_name) in enumerate(zip(periods, rtn_cols)):
        # 计算每个柱子的位置
        offset = (i - (len(periods) - 1) / 2) * bar_width
        positions = indices + offset
        
        # 绘制柱状图
        ax.bar(positions, result_df[f'period{period}'], 
               width=bar_width * 0.9,  # 稍微缩小宽度，让柱子之间有间隙
    ...
    ax.set_xticklabels([f'{int(x)}' for x in result_df.index])
    ...
    # 自动调整y轴范围，为柱状图标签留空间
    y_max = result_df.max().max()
    y_min = result_df.min().min()
...
def process_signals_v2(df, th1, th2,
    ...
    if trading_hours == ["09:30-11:30", "13:00-15:00"]:
        ...
        df['factor_val'] = np.where(
            ((df[ts_col].dt.time >= time(21, 0, 0)) & (df[ts_col].dt.time <= time(21, 10, 0))) |  # 21:00-21:10
            ((df[ts_col].dt.time >= time(0, 50, 0)) & (df[ts_col].dt.time <= time(1, 0, 0))) |   # 00:50-01:00（次日）
            ((df[ts_col].dt.time >= time(9, 0, 0)) & (df[ts_col].dt.time <= time(9, 10, 0))) |   # 9:00-9:10
            ((df[ts_col].dt.time >= time(14, 50, 0)) & (df[ts_col].dt.time <= time(15, 0, 0))),   # 14:50-15:00
            0,
            df['factor_val']
        ...
        for i in range(warmup, len(df)):
            ...
            if open_drop:
                if trading_hours == ["09:30-11:30", "13:00-15:00"]:
                    ...
                    if  time_series.iloc[i] < time(9, 10): 
                        pos.iloc[i] = 0
                        in_long = False
                        in_short = False
                        holding_bars = 0
                    ...
                    if (time_series.iloc[i] < time(9, 10)) or \
                       (time_series.iloc[i] >= time(21, 0) and time_series.iloc[i] < time(21, 10)):
                        ...
                        holding_bars = 0
                        continue
                elif trading_hours == ["09:00-11:30", "13:30-15:00", "21:00-01:30"]:
                    # 期货交易时间段：早上9:10之前和夜盘21:10之前不交易
                    if (time_series.iloc[i] < time(9, 10)) or \
                       (time_series.iloc[i] >= time(21, 0) and time_series.iloc[i] < time(21, 10)):
                        pos.iloc[i] = 0
            ...
            if close_drop:
                if trading_hours == ["09:30-11:30", "13:00-15:00"]:
                    ...
                    # 期货交易时间段：下午14:50之后和夜盘22:50之后不交易
                    if (time_series.iloc[i] > time(14, 50) and time_series.iloc[i] <= time(15, 0)) or \
                       (time_series.iloc[i] >= time(0, 50) and time_series.iloc[i] <= time(1, 00)):
                        pos.iloc[i] = 0        
                        in_long = False
                        in_short = False
                        holding_bars = 0
...
class ModelBacktester:
    ...
    def load_mktdata(self, variety, start_date = '20180101', end_date = '20241231'):
        ...
        data = data[~data['datetime'].str.contains('10:30:00')]
        self.mkt_data = data
    ...
    def load_models(self):
        ...
        for lgb_file in sorted(self.model_dir.glob("*.lgb")):
            try:
    ...
    def calc_pos_table_fee_rate_suocang(self, merged_data, th1, th2, holding_bars):
        ...
        df = merged_data[['datetime', 'factor', 'pos', 'close', self.trading_price_col, 'th_open_long', 'th_close_long', 'th_open_short', 'th_close_short']].copy()
    ...
    def calc_pos_table_fee_rate_normal(self, merged_data):
        print("使用的交易价格", self.trading_price_col)
        df = merged_data[['datetime', 'factor', 'pos', 'close', self.trading_price_col, 'th_open_long', 'th_close_long', 'th_open_short', 'th_close_short', 'date']].copy()
        df[self.trading_price_col] = df[self.trading_price_col].ffill()
        df[['hand', 'cash', 'market_value', 'init_price', 'cost&slippage', 'margin_occupied', 'equity',
    ...
    def backtest(self,
        ...
        if model_name not in self.predictions:
            raise ValueError(f"模型 {model_name} 的预测结果未找到，请先生成预测结果。")
        
        factor_df = self.predictions[model_name].rename(columns={model_name: 'factor'})[[self.instrument_col, 'factor', self.ts_col]]
        factor_df = self.prepare_data(
                    factor_df,
                    self.mkt_data
                )
        # print(factor_df)
        ...
        if v==2:
            merged_data = process_signals_v2(factor_df,
                                            th1=th1,
                                            th2=th2,
        ...
        if v==3:
            merged_data = process_signals_v3(factor_df,
                                            th1=th1,
                                            th2=th2,
                                            factor_col=self.factor_col,
                                            open_drop=open_drop,
        ...
        if self.fee_way == "number" and self.fee_mode == "平今":
            merged_data = self.calc_pos_table_fee_number_normal(merged_data)
