...
if __name__ == "__main__":
    # 等待到达晚上 20:00:00（北京时间），每半小时检查一次
    logger.info("等待到达晚上 19:00:00（北京时间）...")
    while True:
        now = datetime.now(BEIJING_TZ)
        target_time = now.replace(hour=19, minute=0, second=0, microsecond=0)
        if now >= target_time:
            logger.info(f"已到达北京时间 {target_time.strftime('%H:%M:%S')}，开始写入数据")
            break
        diff_seconds = (target_time - now).total_seconds()
        logger.info(f"当前北京时间 {now.strftime('%H:%M:%S')}，距离 19:00:00 还有 {diff_seconds/3600:.1f} 小时，sleep 30分钟...")
