...
def main():
            ...
            for subdir in ['factors', 'predictions', 'json', 'data']:
                ...
                if os.path.isdir(sub_path):
                    for f in os.listdir(sub_path):
                        if f.startswith(('predictions_', 'factors_', 'trading_status_', 'main_min_', '_min_')):
    ...
    for sym in active_symbols:
        ...
        if func_name not in _IMPORTED_FACTOR_FUNCS:
            print(f"  警告: 跳过 {sym}: 未找到 {func_name}")
            continue
        ...
        if not os.path.isdir(model_path) or not any(f.endswith('.lgb') for f in os.listdir(model_path)):
            print(f"  警告: 跳过 {sym}: 模型目录不存在 {model_path}")
            continue
    ...
    try:
        while True:
            ...
            for retry in range(10):
                ...
                print(f"[Orchestrator] 警告: 重试 10 次后数据仍不齐: {not_ready}，继续执行")
            ...
            if total_elapsed > 5.0:
                print(f"警告: 总耗时 {total_elapsed:.2f}s 超过5秒阈值")
        ...
        # 转存当日 save_files 到历史目录（只保留品种数据 + data）
        print("[Orchestrator] 转存当日数据到历史目录...")
        try:
            trade_date = datetime.now(BEIJING_TZ).strftime('%Y%m%d')
            src_dir = os.path.join(current_dir, "save_files")
            dst_dir = os.path.join(current_dir, "history_save_files", trade_date)
            keep_dirs = {'A', 'B', 'C', 'CS', 'LH', 'M', 'P', 'Y', 'data'}
            ...
            if os.path.exists(src_dir) and os.listdir(src_dir):
                ...
                print(f"[Orchestrator] 已转存到 {dst_dir}")
            else:
                print("[Orchestrator] save_files 为空，无需转存")
        except Exception as e:
            print(f"[Orchestrator] 转存失败: {e}")
...
if __name__ == "__main__":
    # 等待到达晚上 20:00:00，每半小时检查一次
    print("[Orchestrator] 等待到达晚上 20:00:00...")
    while True:
        now = datetime.now(BEIJING_TZ)
        target_time = now.replace(hour=20, minute=0, second=0, microsecond=0)
        if now >= target_time:
            print(f"[Orchestrator] 已到达北京时间 {target_time.strftime('%H:%M:%S')}，开始执行策略")
            break
        diff_seconds = (target_time - now).total_seconds()
        print(f"[Orchestrator] 当前北京时间 {now.strftime('%H:%M:%S')}，距离 20:00:00 还有 {diff_seconds/3600:.1f} 小时，sleep 30分钟...")
        time.sleep(1800)  # sleep 30分钟
