...
def run_345(pred_df, th_open, th_close, now_pos, now_holding, max_holding, bar_in_day, window=5, time_config=None):
    ...
    # 使用严格的时间偏移（前1分钟、前2分钟），而非按位置 shift。
    # 防止实时环境下 self.pred_df 索引不连续（某些分钟缺失）导致 shift 错位。
    w = df['weighted']
    prev1 = w.reindex(w.index - pd.Timedelta(minutes=1)).ffill().values
    prev2 = w.reindex(w.index - pd.Timedelta(minutes=2)).ffill().values
    df['weighted_s'] = w * 0.6 + prev1 * 0.3 + prev2 * 0.1
