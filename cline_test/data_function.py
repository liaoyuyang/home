...
class ResampleMethods:
    ...
    def resample_upmean(self, tick_fac):
        ...
        result = merged['factor_value'].where(diff2 >= 0, 0).groupby(grouper).mean()
        result = result.reindex(index=self.valid_index)
        return result.squeeze()
...
class Factor_generator(ResampleMethods):
    ...
    def vcorr10(self, main_symbol, symbol1, symbol2, window=10):
        ...
        # 先对齐到时间索引，再计算 rolling corr（使用预缓存的 set_index 结果）
        v1 = data1['volume']
        v2 = data2['volume']
        df = pd.DataFrame({'v1': v1, 'v2': v2}).reindex(self.valid_index)
        corr = df['v1'].rolling(window=window).corr(df['v2'])
        return corr.values.flatten()
    ...
    def volumediv_diff(self, main_symbol, symbol1, symbol2, window1=20, window2=5):
        ...
        vol1 = data1["volume"]
        vol2 = data2["volume"]
        df = pd.DataFrame({"v1": vol1, "v2": vol2}).reindex(self.valid_index)
        ...
        return vol_ratio_diff.values.flatten()
