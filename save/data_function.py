...
def load_config_for_symbols(config, main_symbol, other_symbols):
    ...
    """
    # 1. 验证主品种是否存在
    main_main_key = f"{main_symbol}_main"
    main_next_key = f"{main_symbol}_next"
    
    if main_main_key not in config["instruments"]:
        raise ValueError(f"配置中找不到主品种 {main_symbol} 的主合约")
    
    # 2. 构建 instrument_list
    instrument_list = []
    
    # 添加主合约（第一个位置）
    instrument_list.append(config["instruments"][main_main_key])
    
    # 添加其他品种合约（按照传入的顺序）
    for symbol in other_symbols:
        other_main_key = f"{symbol}_main"
        if other_main_key in config["instruments"]:
            instrument_list.append(config["instruments"][other_main_key])
        else:
            print(f"警告: 品种 {symbol} 的主合约配置不存在，已跳过")
    
    # 添加主品种的下一个合约（最后一个位置）
    if main_next_key in config["instruments"]:
        instrument_list.append(config["instruments"][main_next_key])
    else:
        print(f"警告: 主品种 {main_symbol} 没有下一个合约配置")
    
    # 获取模型配置
    model_config = config.get("model_config", {}).get(main_symbol, {})
    model_path = model_config.get("load_model_path")
    save_path = model_config.get("save_path")
    publisher_port = model_config.get("port")
    ...
    return {
        ...
        "main_contract": config["instruments"][main_main_key],
        "next_contract": config["instruments"].get(main_next_key),
...
class ResampleMethods:
    ...
    def _prepare_data(self, tick_fac, extra_cols):
        ...
        if extra_cols:
            merged_data = pd.concat([
                tick_fac,
                self.tick_data.set_index('datetime')[extra_cols]
            ], axis=1)
        else:
    ...
    def resample_agg(self, tick_fac, agg_method):
        ...
        # 2. 特殊统计方法
            ...
            result = self._apply_resample(tick_fac, lambda x: x.kurt())
            return ensure_1d_array(result)
        
        elif agg_method == 'skewness':
            result = self._apply_resample(tick_fac, lambda x: x.skew())
            return ensure_1d_array(result)
    ...
    def resample_askdommean(self, tick_fac):
        ...
        return self._apply_resample(tick_fac, compute_askdommean, ['bvall', 'avall'])
    ...
    def resample_biddommean(self, tick_fac):
        ...
        return self._apply_resample(tick_fac, compute_biddommean, ['bvall', 'avall'])
    
    def resample_corrAsk(self, tick_fac):
        """与ask相关性的加权平均"""
        def compute_corrAsk(group):
            weights = 1 - group.corrAskwap
            weighted_value = (group['factor_value'] * weights).sum() / weights.sum() if weights.sum() != 0 else np.nan
            return weighted_value
        
        return self._apply_resample(tick_fac, compute_corrAsk, ['corrAskwap'])
    
    def resample_corrBid(self, tick_fac):
        """与bid相关性的加权平均"""
        def compute_corrBid(group):
            weights = 1 - group.corrBidwap
            weighted_value = (group['factor_value'] * weights).sum() / weights.sum() if weights.sum() != 0 else np.nan
            return weighted_value
        
        return self._apply_resample(tick_fac, compute_corrBid, ['corrBidwap'])
    
    def resample_Mstd(self, tick_fac):
        """M_std加权平均"""
        def compute_Mstd(group):
            weighted_value = (group['factor_value'] * group.M_std).sum() / group.M_std.sum() if group.M_std.sum() != 0 else np.nan
            return weighted_value
        
        return self._apply_resample(tick_fac, compute_Mstd, ['M_std'])
    
    # 3. 条件平均类方法
    def resample_upmean(self, tick_fac):
        """价格上涨时的平均值"""
        def compute_upmean(group):
            weighted_value = np.where(group['mid_price'].diff(2) >= 0, group['factor_value'], 0)
            return weighted_value.mean()
        
        return self._apply_resample(tick_fac, compute_upmean, ['mid_price'])
    
    def resample_downmean(self, tick_fac):
        """价格下跌时的平均值"""
        def compute_downmean(group):
            weighted_value = np.where(group['mid_price'].diff(2) <= 0, group['factor_value'], 0)
            return weighted_value.mean()
        
        return self._apply_resample(tick_fac, compute_downmean, ['mid_price'])
    ...
    def resample_MADmean(self, tick_fac):
        ...
        return self._apply_resample(tick_fac, compute_MADmean, ['mid_price'])
    ...
    def resample_trend_rev(self, tick_fac):
        ...
        return self._apply_resample(tick_fac, compute_trend_rev, ['mid_price'])
    
class Factor_generator(ResampleMethods):
    def __init__(self, tick_data, min_data, other_min1, other_min2, other_min3, min_next):

        self._load_data_tick(tick_data)
        self._load_data_min(min_data, other_min1, other_min2, other_min3, min_next)
        super().__init__(tick_data, self.valid_index)

        # self.symbol_data_map = {
        #     'P': self.min_data,       # 主品种
        #     'M': self.other_min1,     # 第一个其他品种
        #     'A': self.other_min2,     # 第二个其他品种
        #     'Y': self.other_min3,     # 第三个其他品种
        #     'P_next': self.min_next   # 下一个合约
        # }
        
    def load_df_names(self):
        self.symbol_data_map = dict(
            zip(self.dict_keys, [self.min_data, self.other_min1, self.other_min2, self.other_min3, self.min_next])
        )
    ...
    def _load_data_min(self, *min_datas):
        self.min_data, self.other_min1, self.other_min2, self.other_min3, self.min_next = min_datas
        
        # mark
        self.main_multiplier = 10
        
        time_index = (self.tick_data.set_index('datetime')
        ...
        self.valid_index = (self.tick_data.set_index('datetime')
    ...
    def FAC_MPB(self, agg_method='mean'):
        ...
        trade_value_diff = data['TotalTradeValue'].diff()
        trade_volume_diff = data['TotalTradeVolume'].diff()
        TP = np.where(trade_volume_diff != 0,
                     trade_value_diff / (contract_multiplier * trade_volume_diff),  
                     M)
        fac = TP - M
        factor_df = pd.DataFrame(fac.values, index=data.datetime, columns=['factor_value'])
        factor_df = self.resample_agg(factor_df, agg_method)
    ...
    def bar3_trend_corr(self):
        ...
        factor = _rolling_corr(data['close'].values)
    ...
    def term_rtn(self, window=5):
        """
        期限结构因子
        五根线上两个合约收益率之差
        """
        data = self.min_data.copy()
        data_czl = self.min_next.copy()
        data = data.merge(data_czl, on=['datetime'],suffixes=('', '_czl'), how='left')
    ...
    def cvcorr10_diff(self, main_symbol, symbol1, symbol2, window=10):
        ...
        if symbol1 == main_symbol:
            data1 = self.min_data.copy()
        else:
            data1 = self.get_symbol_data(symbol1)
        
        if symbol2 == main_symbol:
            data2 = self.min_data.copy()
        else:
            data2 = self.get_symbol_data(symbol2)

        corr1 = data1['close'].rolling(window=window).corr(data1['volume']).tail(len(self.valid_index))
        corr2 = data2['close'].rolling(window=window).corr(data2['volume']).tail(len(self.valid_index))
        FAC = corr1 - corr2
        FAC_series = pd.Series(FAC.values, index = self.valid_index)        
        return FAC_series.values.flatten()
    ...
    def closepctchg_sub(self, main_symbol, symbol1, symbol2, window=20):
        ...
        pct1 = data1['close'].pct_change(window).tail(len(self.valid_index))
        pct2 = data2['close'].pct_change(window).tail(len(self.valid_index))
    ...
    def oi5_diff(self, main_symbol, symbol1, symbol2, window=5):
        ...
        oi1 = data1['open_interest'].pct_change(window).tail(len(self.valid_index))
        oi2 = data2['open_interest'].pct_change(window).tail(len(self.valid_index))
    ...
    def vcorr10(self, main_symbol, symbol1, symbol2, window=10):
        ...
        corr = data1['volume'].rolling(window=window).corr(data2['volume'])
        corr_tail = corr.tail(len(self.valid_index))
        
        FAC_series = pd.Series(corr_tail.values, index=self.valid_index)
        return FAC_series.values.flatten()
    ...
    def volumediv_diff(self, main_symbol, symbol1, symbol2, window1=20, window2=5):
        ...
        vol_sum1 = data1['volume'].rolling(window=window1).sum()
        vol_sum2 = data2['volume'].rolling(window=window1).sum()
        
        vol_ratio = vol_sum1 / (vol_sum2 + 1e-8)  # 避免除零
        vol_ratio_diff = vol_ratio.diff(window2)
        
        vol_ratio_diff_tail = vol_ratio_diff.tail(len(self.valid_index))
        
        FAC_series = pd.Series(vol_ratio_diff_tail.values, index=self.valid_index)
        return FAC_series.values.flatten()
...
def df_is_trading_time(df: pd.DataFrame, tscol_name: str, trade_type: list, word: str=True):
    ...
    if trade_type == ["09:30-11:30", "13:00-15:00"]:
        # 股票类交易时间
        df['is_trading'] = (
            df[tscol_name].dt.time.between(time(9, 30, 0), time(11, 30, 0)) |
            df[tscol_name].dt.time.between(time(13, 0, 0), time(15, 0, 0))
        )

    elif trade_type == ["09:00-11:30", "13:30-15:00", "21:00-23:00"]:
        # 期货标准交易时间（夜盘到23点）
        df['is_trading'] = (
            df[tscol_name].dt.time.between(time(9, 0, 0), time(10, 15, 0)) |
            df[tscol_name].dt.time.between(time(10, 30, 0), time(11, 30, 0)) |
            df[tscol_name].dt.time.between(time(13, 30, 0), time(15, 0, 0)) |
            df[tscol_name].dt.time.between(time(21, 0, 0), time(23, 0, 0))
        )
        ...
        df['is_trading'] = (
            ...
            df[tscol_name].dt.time.between(time(10, 30, 0), time(11, 30, 0)) |
            df[tscol_name].dt.time.between(time(13, 30, 0), time(15, 0, 0)) |
            df[tscol_name].dt.time.between(time(21, 0, 0), time(23, 59, 59, 999999)) |
            df[tscol_name].dt.time.between(time(0, 0, 0), time(1, 0, 0))
        )
...
def load_fac_df_old(factor_col, instrument_list, recent_data_path, trade_type, dict_keys, generate_factor_dataframe):
    ...
    instrument_next = instrument_list[-1]  # 下一个合约（最后一个位置）
    other_instruments = instrument_list[1:-1]  # 中间的其他品种
    ...
    min_data_next = pd.read_csv(f'{recent_data_path}/{instrument_next}_min.csv', parse_dates=['datetime']).reindex(
        columns=['datetime', 'open', 'high', 'low', 'close', 'volume', 'turnover', 
                'instrument', 'open_interest', 'trade_date', 'LAST_TRADE_DATE']
    )
    ...
    fac_generator_args = [tick_data_main, min_data_main] + other_min_dfs + [min_data_next]
...
def run_345(pred_df, th_open, th_close, now_pos, now_holding, max_holding, bar_in_day=345, time_config=None):
    """
    运行交易策略
    ...
    df['weighted_s'] = df['weighted'] * 0.6 + df['weighted'].shift() * 0.3 + df['weighted'].shift(2) * 0.1
    ...
    print(round(quantile_position*100,2), "%")
    ...
    if in_day_session or in_night_session:
        return now_pos, now_holding, weighted, thresholds_df, df
    else:
        print(f"非交易时间段 {current_time}，强制平仓")
        return 0, 0, {}, pd.DataFrame(), df
