...
def load_data(instrument, date, trade_type, data_sources, last_trade_date_map=None):
    ...
    def pre_resample_data(df, date, instrument):
        ...
        df_trading = df.copy()
        df_trading['highest_price'] = df_trading.groupby('trade_date')['last_price'].cummax()
        df_trading['lowest_price'] = df_trading.groupby('trade_date')['last_price'].cummin()
        df_trading = df[df['is_trading']].copy()
