...
def load_data(instrument, date, trade_type, data_sources, last_trade_date_map=None):
    ...
    def pre_resample_data(df, date, instrument):
        ...
        # 截断 datetime 到 10ms（与研究环境 DataLoader.py 保持一致）
        df['datetime'] = pd.to_datetime(df['datetime']).dt.strftime('%Y-%m-%d %H:%M:%S.%f').str[:-4]
        # 去重：先差分后去重，保留最后一条
...
def get_instruments_to_process(config):
    ...
    return list(set(v['contract'] for v in instruments.values() if isinstance(v, dict) and 'contract' in v))
...
if __name__ == "__main__":
    ...
    # 构建合约 -> 品种代码 映射，用于获取各自的交易时段
    instruments_cfg = config.get('instruments', {})
    instrument_to_symbol = {v['contract']: k for k, v in instruments_cfg.items()}
    ...
    last_trade_date_map = {
        v['contract']: v['last_trade_date']
        for v in instruments_cfg.values()
        if 'last_trade_date' in v
    }
    
    # 并行处理所有合约（每个合约使用自己对应的交易时段）
    results = Parallel(n_jobs=parallel_jobs)(
        delayed(process_single_instrument)(
            instrument, 
            date_lst, 
            recent_data_path,
            get_trade_hours_for_symbol(instrument_to_symbol.get(instrument, ''), config),
            data_sources,
            last_trade_date_map,
            tick_cache_days
        )
