...
def _load_kline(symbol: str, save_root: str, replay_mode: Optional[dict] = None) -> list:
    """读取当日完整行情（含前一夜盘），剔除异常 bar"""
    data_dir = os.path.join(save_root, symbol, 'data')
    if replay_mode and replay_mode.get('enabled'):
        date_str = replay_mode.get('day_date', '')
        prev_trade_day = replay_mode.get('night_date', '')
    else:
...
class TradeMonitor:
    def __init__(self, save_root: str, symbols: list, clear_on_start: bool = True, replay_mode: Optional[dict] = None):
        self.save_root = save_root
        self.symbols = symbols
        self.data_dir = os.path.join(save_root, 'data')
        self.monitor_dir = os.path.join(save_root, '盘中分析')
        os.makedirs(self.monitor_dir, exist_ok=True)
        self.replay_mode = replay_mode or {}
        if self.replay_mode.get('enabled'):
            print(f"[Monitor] 回放模式已启用 | 白天盘日期: {self.replay_mode.get('day_date')} | 夜盘日期: {self.replay_mode.get('night_date')}")
    ...
    def _save_trades(self, symbol: str):
        if self.replay_mode.get('enabled'):
            date_str = self.replay_mode.get('day_date', '').replace('-', '')
        else:
    ...
    def _get_scan_dates(self) -> list:
        """返回需要扫描的日期列表（回放模式用固定日期，实盘用今天）"""
        if self.replay_mode.get('enabled'):
            day = self.replay_mode.get('day_date', '')
            night = self.replay_mode.get('night_date', '')
            return [d for d in [night, day] if d]
        else:
            ...
            for symbol in self.symbols:
                try:
                    ...
                    if self.processed[symbol]:
                        ...
                        if not any(sample.startswith(d) for d in scan_dates):
                    ...
                    files = []
                    for d in scan_dates:
                        files.extend(glob.glob(os.path.join(json_dir, f'trading_status_{d}*.json')))
    ...
    def run(self, poll_interval: float = 5.0, skip_history: bool = True):
        ...
        if skip_history:
            ...
            print(f"[Monitor] 启动（跳过历史复盘）| 品种: {self.symbols} | 扫描日期: {scan_dates}")
            for symbol in self.symbols:
                try:
                    json_dir = os.path.join(self.save_root, symbol, 'json')
                    if not os.path.exists(json_dir):
            ...
            print(f"[Monitor] 启动复盘 | 品种: {self.symbols} | 扫描日期: {scan_dates}")
            for symbol in self.symbols:
                try:
                    json_dir = os.path.join(self.save_root, symbol, 'json')
                    if not os.path.exists(json_dir):
...
def index():
    ...
    if selected_sym != 'ALL' and selected_sym in SYMBOLS and save_root:
        replay = state.get('replay_mode', {})
        kline_data = _load_kline(selected_sym, save_root, replay)
...
if __name__ == '__main__':
    ...
    monitor = TradeMonitor(save_root, SYMBOLS, clear_on_start=True, replay_mode=replay_mode)
