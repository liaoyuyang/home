...
def parse_time_from_update_time(update_time: Any) -> datetime:
    ...
    try:
        ...
        import os
        playback_date_str = os.environ.get('PLAYBACK_DATE')
        if playback_date_str:
            today = datetime.strptime(playback_date_str, '%Y-%m-%d').date()
        else:
...
class RealTimeMarketData:
    ...
    def __init__(self, instruments: List[str], db_path: Optional[str] = None, 
                 ...
                 replay_mode: Optional[dict] = None):
        self.instruments = instruments
        self.instrument_set = set(instruments)
        self.replay_mode = replay_mode or {}
        if self.replay_mode.get('enabled'):
            logger.info(f"[Replay] 回放模式已启用 | day_date={self.replay_mode.get('day_date')} | night_date={self.replay_mode.get('night_date')}")
    ...
    def _handle_market_data(self, data: Dict[str, Any]) -> None:
        ...
        try:
            ...
            # 回放模式：替换日期（hour>=18 用 night_date，<18 用 day_date）
            if self.replay_mode.get('enabled'):
                hour = tick_datetime.hour
                replace_date_str = (
                    self.replay_mode.get('night_date') if hour >= 18
                    else self.replay_mode.get('day_date')
                )
                if replace_date_str:
                    replace_date = datetime.strptime(replace_date_str, '%Y-%m-%d').date()
                    tick_datetime = tick_datetime.replace(
            ...
            # 回放模式：trade_date 固定用 day_date（全天统一），不改 datetime 的 hour 逻辑
            if self.replay_mode.get('enabled'):
                trade_date = self.replay_mode.get('day_date', tick_datetime.strftime('%Y-%m-%d'))
...
def get_instruments_from_config(config: Dict[str, Any], exchange: str) -> List[str]:
    """从配置中获取合约列表"""
    return [
        config["instruments"][symbol]["contract"]
        for symbol in config.get("symbols", [])
        if symbol in config.get("instruments", {})
    ]
